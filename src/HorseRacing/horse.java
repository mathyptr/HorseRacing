...
/**
 * Classe per l'implementazione del cavallo: gestisce il suo spostamento e il
 * caricamento delle sue immagini da file per la gestione dell'animazione
 * 
...
public class horse extends Thread {
	...
	/**
	 * metodo per l'inizializzazione di tutte le possibili velocità associabili ad
	 * un cavallo
	 *
	 */
	private void initDefaultSpeed() { // le seguenti velocità verranno poi assegnate a ciascun cavallo in modo random
		speedhorse.add(14);
		speedhorse.add(15);
		speedhorse.add(18);
		speedhorse.add(10);
	...
	/**
	 * metodo per il caricamento delle immagini di un cavallo: queste permetteranno
	 * la sua animazione
	 * 
	 * @exception e
	 */
	...
	public void move() {
		int n;
		n = posx + 1 + speed; // viene incrementato solo se il cavallo non e' ancora giunto al traguardo
		if (posx < end)
			if (n > end)
				posx = end;
			else
	...
	public void run() {
		while (posx < end) {
			step.get(); // cerco di accedere alla sezione critica per effettuare lo spostamento
			move();
			try {
				sleep(500);
